...
void FormPeriodicDataPkt(void) {
    ...
    Msdu[i++] = 11;//BatteryVoltage;
    Msdu[i++] = 45;//BatteryVoltage >> 8;
    Msdu[i++] = Temperature;
    Msdu[i++] = AccRawData.x;
    ...
    //ECC
    //Encrypted message stores in array Cipher[] of type unsigned char
    EncryptArray(Msdu);
}
...
int main(void) {
    ...
    // On power on, after all initialization, send a DATA PACKET (try to include public key)
    /*
    ...
    */
    ...
    T8 = MCPS_DATA_REQUEST(MAC_ADDR_MODE_SHORT, MacPANID, (unsigned char *) &MacShortAddress, MAC_ADDR_MODE_SHORT, MacPANID, (unsigned char *) &MacCoordShortAddress, CipherLength, &Cipher[0], 1, 0x1);         
